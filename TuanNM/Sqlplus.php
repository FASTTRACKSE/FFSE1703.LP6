...
Bai 5
SELECT sinh_vien.ma_sv , sinh_vien.fullname , (YEAR(CURRENT_DATE) - YEAR(sinh_vien.birthday)) AS tuoi,bs1.diem AS BS1, bs2.diem AS BS2,bs3.diem AS BS3,AVG(dhbs.diem) AS diem_TB_bo_sung FROM ((((mon_hoc_bo_sung mhbs 
RIGHT JOIN diem_hoc_bo_sung dhbs ON mhbs.ma_mh = dhbs.ma_mh) LEFT JOIN sinh_vien ON sinh_vien.ma_sv = dhbs.ma_sv ) LEFT JOIN diem_hoc_bo_sung bs1 ON sinh_vien.ma_sv = bs1.ma_sv AND bs1.ma_mh = 1) LEFT JOIN diem_hoc_bo_sung bs2 ON sinh_vien.ma_sv = bs2.ma_sv AND bs2.ma_mh = 2) LEFT JOIN diem_hoc_bo_sung bs3 ON sinh_vien.ma_sv = bs3.ma_sv AND bs3.ma_mh = 3
GROUP BY dhbs.ma_sv 